...
barrios_geo <- left_join(barrios_geo, df_casos_confirmados_barrios, by = "barrio")
barrios_geo <- left_join(barrios_geo, df_fallecidos_barrios, by = "barrio")
...
comunas %>% 
  ...
  geom_sf_text(aes(label = comuna, family = "sans")) +
...
ggplot() +
  geom_sf(aes(fill = tasa_letalidad)) +
  geom_sf_text(aes(label = barrio), size = 3) +
